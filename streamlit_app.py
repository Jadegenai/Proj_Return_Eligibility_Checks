...
def initialize_session_state():
    ...
    defaults = {
        ...
        "order_id": None,
        "order_products": None,
        "awaiting_phone": True,  # Track if we're waiting for the phone number
        "awaiting_order_id": False,  # Track if we're waiting for the order ID
        "awaiting_product": False  # Track if we're waiting for the product selection
    }
...
def create_snowflake_connection():
    ...
    try:
        ...
        snowflake_database = "RETURN_REQUEST_DB"
        snowflake_schema = "RETURN_REQUEST_SCHEMA"
        snowflake_warehouse = "COMPUTE_WH"
        snowflake_role = "ACCOUNTADMIN"

        # Build the connection URI using the defined variables
        connection_uri = (
...
def get_order_products(order_id: str, customer_id: str, engine) -> dict:
    ...
    try:
        ...
        logging.error(f"Order products error: {e}")
...
def check_return_eligibility(product_id: int, engine) -> dict:
    ...
    try:
        ...
        logging.error(f"Return eligibility error: {e}")
...
def normalize_product_input(user_input: str, products: dict) -> int:
    ...
    try:
        ...
        logging.error(f"Product identification error: {e}")
...
def main():
    ...
    if user_input := st.chat_input("Your message:"):
        ...
        try:
            ...
            elif "return" in user_input.lower() or "help" in user_input.lower() or "hello" in user_input.lower() or "hey" in user_input.lower():
            ...
            elif st.session_state.awaiting_phone and re.search(r'\d', user_input):
            ...
            # 4. Order ID handling
            elif st.session_state.awaiting_order_id and user_input.isdigit():
            ...
            # 5. Product identification and return check
            elif st.session_state.awaiting_product:
                ...
                if product_id:
                    ...
                    response = "🔍 I'm sorry, I couldn't identify the product. Please try again."
            ...
            logging.error(f"Error in main conversation flow: {e}")
